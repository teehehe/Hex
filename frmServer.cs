...
namespace Hex
{
    public partial class frmServer : KryptonForm
    {
        public frmServer()
        {
            InitializeComponent();
        }
        private void btnAtlas_Click(object sender, EventArgs e)
        {
            this.TopMost = false;
            this.Enabled = false;
            ...
            if (DAoCFolder == null)
                DAoCFolder = @"C:\Program Files (x86)\Electronic Arts\Dark Age of Camelot";
            Hex.ISelect select = new Select
            {
                InitialFolder = DAoCFolder
            };
            select.ShowDialog();
            DAoCFolder = select.Folder;
            if (DAoCFolder != null)
            MyIni.Write("AtlasPath", DAoCFolder);
            this.Enabled = true;
        }
        ...
        private void btnLive_Click(object sender, EventArgs e)
        {
            ...
            DAoCFolder = select.Folder;
            if (DAoCFolder != null)
            MyIni.Write("LivePath", DAoCFolder);
            this.Enabled = true;

        }
        private void btnCelestius_Click(object sender, EventArgs e)
        {
            this.TopMost = false;
            this.Enabled = false;
            IniFile MyIni = new("Settings.ini");
            ...
            DAoCFolder = select.Folder;
            if (DAoCFolder != null)
            MyIni.Write("CelestiusPath", DAoCFolder);
            this.Enabled = true;
